...
def detect(opt):
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        # 모든 id와 좌표 저장
                        temp = [output[4], output[5], output[0], output[1], output[2] - output[0],
                                output[3] - output[1]]
                        print(temp)
                        if (output[5] == 0):
                            person.append(temp)
            ...
            if len(before) != 0:
                for b in before:
                    if b not in now:
                        ...
                        trash.append(b[1])  # ID만 저장
                        Practice.res = True

            # 새롭게 나타난 클래스(사람 제외) object id 확인, 시간 10초 및 좌표 그대로 체크
            for o in object:
                if o in b_object:
                    checkLong = checkLong + 1
                    print("물체 좌표 고정 시간:" + str(checkLong))
    ...
    # 유사 행위 확인 trash와 object 비교 res = False
    for o in object:
        if o[0] not in trash:
            object.remove(o)
